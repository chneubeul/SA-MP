...
stock Float:CalculateObjectDistance(objectid){
	switch(objectid){
		case
...
public OnFilterScriptInit(){
	...
	WriteLogEx(LOG_FILE," ");
	WriteLogEx(LOG_FILE,"[IMPORTANT] Trwa przeladowywanie obiektow...");
	WriteLogEx(LOG_FILE," ");
	...
	for(new i = 0, j = sizeof(DoTFile); i < j; i++){
		if(fexist(DoTFile[i])){
			...
			if(edi != ERROR_CODE_ADM_OK){
				printf("DoT '%s' ³adowanie nie powiod³o siê, kod: %d",DoTFile[i],ADMToInt(edi));
			}
		}
	}
	for(new i = 0, j = sizeof(BinIMGFile); i < j; i++){
		if(fexist(BinIMGFile[i])){
			...
			if(edi != ERROR_CODE_BINLDR_OK){
				printf("IMG '%s' ³adowanie nie powiod³o siê, kod, %d",BinIMGFile[i],IMGToInt(edi));
			}
		}
	}
	if(ecnt > 0){
